...
impl FFmpegUtils {
        ...
        F: Fn(f32),
    {
        ...
        progress_callback(0.1); // Начало анализа
        ...
        // Если нужно исправление VFR, делаем это первым
        progress_callback(0.2);
        if let Some(ref temp_path) = temp_file {
            self.fix_vfr(&input_path, temp_path, |progress| {
                progress_callback(0.2 + progress * 0.3); // от 20% до 50%
            })?;
        }
        ...
        self.compress_video_core_with_progress(
            current_input,
            &output_path,
            output_format,
            crf_value,
            |progress| {
                progress_callback(0.5 + progress * 0.4); // от 50% до 90%
            },
        )?;
    ...
    fn fix_vfr<F>(&self, input_path: &Path, output_path: &Path, progress_callback: F) -> Result<(), String>
    where
        F: Fn(f32),
    {
        // Исправление VFR - простая реализация без отслеживания прогресса для простоты
        progress_callback(0.5); // Показываем, что процесс идёт
        ...
        progress_callback(1.0); // Завершен
    ...
    fn compress_video_core_with_progress<F>(
        &self,
        input_path: &Path,
        output_path: &Path,
        output_format: OutputFormat,
    ...
    where
        F: Fn(f32),
    {
        let _config = AppConfig::default(); // Подавляем предупреждение
        ...
        // Показываем, что процесс компрессии начинается
        ...
        // Показываем, что процесс продолжается
        progress_callback(0.9);

        match output {
            Ok(_) => {
                progress_callback(1.0); // Завершён
                Ok(())
            },
